...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  $r = mysql_db_query($db,"select * from order_details") or die ("phpmyadmin - " . $create_table_sql . "<br/>" . mysql_error());
  echo "<table>";
  while ($o = mysql_fetch_object($r)) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=salary size=20 value='". $o->Quantity. "' $chg;text-align:right'></td>
...
# ส่วนเพิ่มข้อมูล
if ($act == "add") {
  $q  = "insert into order_details (OrderID,ProductID,Quantity)values('". $_REQUEST{'id'} . "','". $_REQUEST{'ns'} . "','". $_REQUEST{'salary'} . "')";
  $conn = mysql_connect("$host","$user","$password");
...
# ส่วนลบข้อมูล
if ($act == "del : confirm") {
  $q  = "delete from order_details where OrderID ='". $_REQUEST{'id'} . "'";
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
...
# ส่วนแก้ไขข้อมูล
if ($act == "edit : confirm") {
  $q  = "update $tb set Quantity ='". $_REQUEST{'ns'} . "', ProductID ='". $_REQUEST{'salary'} . "' where OrderID=" . $_REQUEST{'id'};
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
